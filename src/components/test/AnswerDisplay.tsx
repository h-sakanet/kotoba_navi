...
import { isHomonymCategory, isProverbGroupCategory } from '../../utils/categoryMeta';
            ...
            {isHomonymCategory(currentWord.category) ? (
                                ...
                                {isProverbGroupCategory(currentWord.category) ? (
                                    ...
                                    <>
                                        ...
                                        {member.exampleSentence && (
                                            <div className="text-gray-400 text-sm font-bold pl-1 mb-1">
                                                {member.exampleSentence}
                                            </div>
                                        )}
                                        {/* Proverb (Main) */}
                                        <div className="font-bold text-blue-600 text-2xl md:text-3xl text-left pl-1 leading-relaxed w-full">
                                            {member.rawWord}
                                        </div>
                                    </>
                                ) : (
                ...
                <div className="flex w-full gap-4">
                    {currentWord.groupMembers.map((member, idx) => (
                        <div key={idx} className="flex-1 flex flex-col items-center justify-center border-r last:border-0 border-blue-100 px-4">
                            {member.yomigana && (
                                <div className="text-gray-400 text-sm font-bold mb-1">{member.yomigana}</div>
                            )}
                ...
                <>
                    ...
                    <h2
                        className={clsx(
                            "font-bold text-blue-600 leading-snug",
